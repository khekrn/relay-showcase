 ...
 * taskName. The single record parameter binds the whole node input; the optional {@link TaskContext} carries
 * metadata (attempt, execution/state ids) and the Dynamic-state hooks {@code routeTo}/{@code runSubgraph}.
 */
@Component
public class Tasks {

    @Step("handle")
    Handled handle(HandleIn in, TaskContext c) {
        return new Handled(in.action() != null ? in.action() : "count=" + in.count());
    }

    @Step("process")
    ProcResult process(ProcOne in, TaskContext c) { return new ProcResult(in.id(), true); }

    @Step("quote")
    Quote quote(QuoteIn in, TaskContext c) { return new Quote(in.provider(), 100 + in.provider().hashCode() % 50); }

    @Step("tick")
    Ticked tick(Tick in, TaskContext c) { return new Ticked(in.n() < 2, in.n()); }   // loop 3× (n=0,1,2)

    @Step("flaky")
    FlakyOut flaky(FlakyIn in, TaskContext c) {
        if (c.attempt() < 3) throw new Boom("transient failure on attempt " + c.attempt());
        return new FlakyOut("OK", c.attempt());
    }

    @Step("reserve")
    Reservation reserve(ReserveIn in, TaskContext c) { return new Reservation("RES-" + Math.abs(in.amount()), in.amount()); }

    @Step("release")   // compensation for reserve
    Released release(Reservation in, TaskContext c) {
        System.out.println("        ↩ compensating: released " + in.ref() + " ($" + in.amount() + ")");
        return new Released(in.ref(), true);
    }

    @Step("boom")
    Handled boom(BoomIn in, TaskContext c) { throw new Boom("hard failure for " + in.who()); }

    @Step("job")
    JobOut job(JobIn in, TaskContext c) { return new JobOut(in.name(), "DONE"); }

    @Step("classify")   // a Dynamic ROUTE task: picks the next state at runtime via ctx.routeTo(...)
    Handled classify(Amount in, TaskContext c) {
    ...
    ItemOut maybe(ItemIn in, TaskContext c) {
    ...
    Handled plan(PlanIn in, TaskContext c) {
